...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "projet-ear.ear");

        // Current directory is the root of the "projet-web" project. Go up one level
        // and enter the "projet-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../projet-ejb/target/projet-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "projet-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/projet-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "projet-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
